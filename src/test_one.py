...
def main():
    ...
    parser.add_argument('--pretrained_model', type=str, default='synthesis', help='synthesis_RGB, synthesis_Y, removal_blind_RGB, removal_blind_Y ,removal_non_blind_RGB, removal_non_blind_Y ')
    ...
    if "removal" in args.pretrained_model:
        image_L, original_shape = process_input(args.input_path, True)
        if "non_blind" in args.pretrained_model:
            ...
            image_E = model(image_L, training = True)
        ...
        imageio.imsave( args.output_path +os.path.basename(args.input_path), image_E) 


    if "synthesis" in args.pretrained_model:
        image_L, original_shape = process_input(args.input_path, False)
        shape = image_L.shape
        X_levels = np.empty((1, shape[1], shape[2], 1), dtype=np.float32)
        X_levels[0,:,:,:] = np.full((shape[1], shape[2], 1),  args.level)
